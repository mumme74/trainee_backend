...
import { transformUser, lookupUser } from "./users";
...
export const composeErrorResponse = 
      (err: Error): IGraphQl_ErrorResponse => 
  {
      ...
      try {
        if (opt.anyOf && 
            !(Array.isArray(opt.anyOf) ? 
              opt.anyOf.find(any=>authReq.user.roles.indexOf(any)> -1) !== undefined :
        ...
        if (opt.exclude &&
            ...
            opt.exclude.find(any=>authReq.user.roles.indexOf(any)> -1) !== undefined :
            opt.exclude in authReq.user.roles))
        {
          req.res.status(403);
          throw new UserError("You had a priviledge that you should not have to do this");
        }
  
        if (opt.allOf &&
          (Array.isArray(opt.allOf) ? 
          opt.allOf.map(any=>authReq.user.roles.indexOf(any)> -1).length ===  opt.allOf.length:
