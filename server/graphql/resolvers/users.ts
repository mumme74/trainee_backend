...
import User, { IUserCollection, rolesAvailable, rolesAvailableKeys } from "../../models/user";
import { IGraphQl_UserCreateStudentInput, IGraphQl_UserType } from "../schema/users";
...
export const lookupUser = async (userId: string): 
...
export const transformUser = (user: IUserCollection): 
            IGraphQl_UserType => 
{
...
export default {
    ...
    users: rolesFilter(
        ...
        async ({ids}: {ids:string[]}, req: AuthRequest): Promise<IGraphQl_UserType[]> => 
    {
        try {
            ...
            if (req.user.roles.indexOf(rolesAvailable.super) > -1)
                return res;
    ...
    userCreateStudent: rolesFilter(
        ...
        async ({newUser}: {newUser: IGraphQl_UserCreateStudentInput}, req: AuthRequest): 
    ...
    userChangeRoles: rolesFilter(
        {anyOf:[rolesAvailable.admin, rolesAvailable.super]},
         async ({id, roles}: {id: string, roles: string[]}, req: AuthRequest): 
                            Promise<IGraphQl_MutationResponse> =>
    {
        try {
            // string role names to available Roles
            // using set here to ensure we only have one of each type
            const newRoles = [...new Set(roles.map((role: string)=>{
                const idx = rolesAvailableKeys.indexOf(role);
                if (idx < 0) throw new UserError("Role not valid")
                return idx;
            }))];

            if (newRoles.indexOf(rolesAvailable.super) > -1 && req.user.roles.indexOf(rolesAvailable.super) < 0) {
                throw new UserError("Can't set super admin role when you are not super admin.\n Insufficient credentials.")
            }
    ...
    userMoveToDomain: rolesFilter(
        {anyOf:[rolesAvailable.admin, rolesAvailable.super]}, 
        async ({id, domain}: {id: string, domain?: string}, req: AuthRequest): 
        Promise<IGraphQl_MutationResponse> => 
    {
        try {
            if (!domain)
            ...
            if (req.user.roles.indexOf(rolesAvailable.super) < 0) {
                ...
                if (domain !== req.user.domain && domain !== "") 
                    throw new UserError("You dont have priviledges to move user to another domain than your own") 
            }
    ...
    userSetSuperUser: rolesFilter({anyOf: rolesAvailable.super}, async({id}: {id: string}): 
        Promise<IGraphQl_MutationResponse> => 
    {
        try {
    ...
    userDeleteUser: rolesFilter({anyOf:[rolesAvailable.admin, rolesAvailable.super]}, 
        async ({id}: {id: string}, req: AuthRequest): 
                        Promise<IGraphQl_MutationResponse> => 
    {
        try {
            // filter out so we can't delete a user outside of our domain
            const domainFilter = req.user.roles.indexOf(rolesAvailable.super) < 0 ?
                                    {domain:{"$eq": req.user.domain || ''}} : undefined;
            const res = await User.deleteOne({
                "_id": new mongoose.Types.ObjectId(id),
                domain: domainFilter?.domain
            });

            if (!res || res.n < 1) throw new UserError("User not found, could not delete.");
