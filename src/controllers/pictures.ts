...
export type { IPicturesController }

const PicturesController: IPicturesController = {
  getPicture: async (req: Request, res: Response,nex: NextFunction) => {
    const pic = await Picture.findByPk(req.params.path);
    if (pic) {
      if (pic.mime.startsWith('remote'))
        return res.redirect(pic.blob.toString());

      res.setHeader('Content-Type', pic.mime);
      if (process.env.NODE_ENV === "production")
        res.setHeader('Cache-Control', 'max-age=604800'); // 1 week

      return res.send(pic.blob);
    }

    // not found
    return res.send(notFoundImage());
  }
}
